    ...
    public class FMM_StdFinBRHelpers
    {
        ...
        public XFSelectionChangedTaskResult Proc_ModelGrps(SessionInfo si, BRGlobals globals, object api, DashboardExtenderArgs args)
        {
            ...
            var sql = $@"Select Grp.CubeID, Assign.Sequence, Act.CalcType, Model.Name
						...
						AND Assign.ModelGrpID = Model.ModelConfigID
            ...
            //If Table then call Proc_TableModels(si, globals, api, args, CubeID, ModelConfigID)

            return returnResult;
        }

        #region "Process Table Models"
        public XFSelectionChangedTaskResult Proc_TableModels(SessionInfo si, BRGlobals globals, object api, DashboardExtenderArgs args, int cubeID, int ModelConfigID)
        {
            var returnResult = new XFSelectionChangedTaskResult();
            var sql = $@"Select *
